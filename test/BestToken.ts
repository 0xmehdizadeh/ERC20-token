...
describe("BestToken", function () {
  ...
  beforeEach(async function () {
    ...
    BestToken = await ethers.deployContract("BestToken", [
      "BestToken",
      "BST",
      owner.address,
      "1000000",
  ...
  it("Not-owner cannot call mint function", async function () {
    await expect(BestToken.connect(user1).mint(user1.address, 1000))
  ...
  it("Non-owner cannot pause and unpause", async function () {
    await expect(BestToken.connect(user1).pause())
      .to.be.revertedWithCustomError(BestToken, "OwnableUnauthorizedAccount")
      .withArgs(user1.address);
  ...
  it("Wrong address tries to accept the ownership", async function () {
    ...
    await expect(BestToken.connect(user2).acceptOwnership())
      .to.be.revertedWithCustomError(BestToken, "OwnableUnauthorizedAccount")
  ...
  // The following tests exercise EIP-2612 `permit()`, which lets a token holder
  // approve a spender via an off-chain signature instead of an on-chain `approve()` tx
  // (useful for gasless approvals). The signer signs EIP-712 typed data with three parts:
  //
  // - domain: binds the signature to this specific contract + chain, preventing
  //   the same signature from being replayed against a different contract or network
  // - types: describes the shape of the `Permit` struct being signed
  // - message: the actual values the owner is approving (spender, amount, deadline, nonce)
  //
  // The contract recovers the signer from (domain + types + message + signature) and
  // checks it matches `owner`, then applies the approval exactly like a normal approve().
  ...
  it("Permit expires after the deadline", async function () {
    ...
    await expect(
      ...
      .to.be.revertedWithCustomError(BestToken, "ERC2612ExpiredSignature")
      .withArgs(deadline);
  });
  ...
  it("Permit reverts with wrong signer", async function () {
    ...
    await expect(
      ...
      .to.be.revertedWithCustomError(BestToken, "ERC2612InvalidSigner")
